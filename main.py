...
def build_star_schema(df: pd.DataFrame) -> None:
    ...
    df = df.copy()
    # Normalize customer_id: remove trailing .0 for numeric IDs and keep as string
    ...
    # Map surrogate keys to fact table
    fact_df = df[[
        "invoice_no",
        "quantity",
        "unit_price",
    ...
    # Ensure fact natural key components are present; drop rows with null customer_id
    ...
    # Join date_key via a clean helper column
    ...
    assert set(sales_fact["product_id"]).issubset(set(product_dim["product_id"])), (
        "sales_fact.product_id contains values not in product_dim.product_id"
    )
    assert set(sales_fact["customer_sk"]).issubset(set(customer_dim["customer_sk"])), (
        "sales_fact.customer_sk contains values not in customer_dim.customer_sk"
    )
    assert set(sales_fact["date_key"]).issubset(set(date_dim["date_key"])), (
        "sales_fact.date_key contains values not in date_dim.date_key"
    )
    ...
    # Delta load to Postgres using natural keys
    engine = get_pg_engine()
    with engine.begin() as conn:
        ensure_tables(conn)

        # Upsert dimensions based on provided delta keys
        upsert_dimension(
            conn,
        ...
        # Fetch UUID mappings from DB after upserts
...
def ensure_tables(conn) -> None:
    ...
    # Attempt to migrate existing tables' key columns to UUID types if they were TEXT
    # Swallow errors if types are already UUID or incompatible rows exist
    conn.execute(text(
        """
        DO $$ BEGIN
            BEGIN
...
def upsert_dimension(
    ...
    # Ensure primary key values are hyphenated for native UUID type compatibility
    rows_uuid = []
    for r in rows:
        nr = dict(r)
        ...
        rows_uuid.append(nr)
    # Filter out rows with null/empty delta keys to respect NOT NULL and unique constraints
...
def upsert_fact(conn, table_name: str, rows: List[Dict]) -> None:
    if not rows:
        return
    print(f"Upserting {len(rows)} rows into {table_name}...")
    # Normalize UUID columns to hyphenated form for native UUID types
    rows_uuid = []
    for r in rows:
        nr = dict(r)
        ...
        rows_uuid.append(nr)
    # Filter out rows with missing natural key components
...
def _clean_customer_id_series(s: pd.Series) -> pd.Series:
    """Normalize customer_id to string without trailing .0; retain non-numeric as-is; set nulls for missing."""
    # Start as string to preserve non-numeric values
    s_str = s.astype("string")
    # Identify numeric values (including float-like strings)
    s_num = pd.to_numeric(s_str, errors="coerce")
    mask_num = s_num.notna()
    # Create result initialized to original strings
    result = s_str.copy()
    # For numeric, cast to pandas Int64 then to string (removes .0)
